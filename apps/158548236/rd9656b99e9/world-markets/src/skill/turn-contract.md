...
1. **Classify** — first match wins:
   whole-message token (`b` `p` `r` `a`) → LOOKUP ·
...
template > budget. An unknown asset is CANT, not Ask. "Never ask what they meant" is a LOOKUP rule only; an ACTION turn may ask
