...
- `.aomi/config.json` declares platform `world-market-apps` and lists `aomi.toml`.
- `aomi-sdk` pinned EXACTLY (`= x.y.z`) to the target platform's
  `required_sdk_version` (see `platform.json` in `aomi-labs/world-market-apps`).
- `crate-type = ["cdylib"]`.
